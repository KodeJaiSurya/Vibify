...
class DataProcessor:
    ...
    def __init__(self, 
        ...
        self.temp_dir = Path("temp_chunks")  # Changed to Path object for consistency
    ...
    def save_chunk_to_gcs(self, X: np.ndarray, y: np.ndarray, chunk_idx: int) -> Tuple[str, str]:
        """Saves a processed chunk directly to Google Cloud Storage"""
        self.logger.info(f"Saving chunk {chunk_idx} to GCS")
        
        # Define GCS paths
        X_path = f"{self.temp_dir}/X_chunk_{chunk_idx}.npy"
        y_path = f"{self.temp_dir}/y_chunk_{chunk_idx}.npy"
        
        try:
            # Save X array
            with tempfile.NamedTemporaryFile() as temp_x:
                np.save(temp_x.name, X)
                temp_x.seek(0)  # Reset file pointer to beginning
                x_blob = self.bucket.blob(X_path)
                x_blob.upload_from_filename(temp_x.name)
                
            # Save y array
            with tempfile.NamedTemporaryFile() as temp_y:
                np.save(temp_y.name, y)
                temp_y.seek(0)  # Reset file pointer to beginning
                y_blob = self.bucket.blob(y_path)
                y_blob.upload_from_filename(temp_y.name)
                
            self.logger.debug(f"Chunk {chunk_idx} saved to GCS at {X_path} and {y_path}")
            return X_path, y_path
            
        except Exception as e:
            self.logger.error(f"Error saving chunk {chunk_idx} to GCS: {str(e)}")
            raise
    
    def process_all_chunks(self, file_path: str) -> List[Tuple[Path, Path]]:
        """Process all chunks and save directly to GCS"""
        ...
        try:
            for chunk_idx, chunk in enumerate(self.load_data_chunks(file_path)):
                ...
                if X is not None and y is not None:
                    # Save directly to GCS and store the paths as Path objects
                    x_path, y_path = self.save_chunk_to_gcs(X, y, chunk_idx)
                    chunk_paths.append((Path(x_path), Path(y_path)))
                    
                    self.logger.info(f"Successfully processed and saved chunk {chunk_idx + 1}")
            
            self.logger.info(f"All chunks processed and saved. Total chunks: {len(chunk_paths)}")
            return chunk_paths
            
        except Exception as e:
            self.logger.error(f"Error in process_all_chunks: {str(e)}")
            raise